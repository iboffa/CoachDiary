...
import { ExerciseToken, ExerciseTokenType } from './exercise-editor.models';

type ExerciseInteractiveToken = Omit<InteractiveToken, 'type'> & { type: ExerciseTokenType };

export interface ExerciseEditorCanvasEventBindings {
  canvas: Canvas;
  getActiveTool: () => Tool;
  getIsDrawing: () => boolean;
  getCurrentPhasePaths: () => PhasePath[];
  getTokens: () => ExerciseToken[];
...
export function bindExerciseEditorCanvasEvents(bindings: ExerciseEditorCanvasEventBindings): void {
    ...
    if (bindings.getActiveTool() === 'select' && event.target && mouseDownPoint) {
      ...
      if (distance < 5) {
        ...
        const token = resolveInteractiveToken(
          bindings.getTokens() as unknown as PlayerToken[],
          bindings.getShadowTokens(),
          bindings.getCurrentPhasePaths(),
          event.target,
        ) as ExerciseInteractiveToken | null;

        if (token) {
          bindings.clearPathEditing();
          bindings.setSelectedPathObject(null);

          // Cones do not participate in actions
          if (token.type === 'cone') return;

          const passFromId = bindings.getPendingPassFrom();
          if (passFromId) {
            if (token.id !== passFromId) {
          ...
          bindings.setContextMenu({
            ...
            playerId: token.id,
            isBallCarrier: bindings.getBallCarrierIds().includes(token.id),
            hasNearbyPlayer: hasNearbyPlayer(
              bindings.getTokens() as unknown as PlayerToken[],
              bindings.getCurrentPhasePaths(),
              token.id,
            ),
